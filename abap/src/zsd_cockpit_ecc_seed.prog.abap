...
START-OF-SELECTION.
  ...
  PERFORM build_customers.
  PERFORM build_orders.
  PERFORM build_items.
  PERFORM build_docflow.
  PERFORM save_demo_data.
  PERFORM write_summary.

FORM clear_demo_data.
...
FORM write_summary.
  ...
  DATA lv_vbak_count TYPE i.
  DATA lv_vbap_count TYPE i.
  DATA lv_vbfa_count TYPE i.

  DESCRIBE TABLE lt_kna1 LINES lv_kna1_count.
  DESCRIBE TABLE lt_vbak LINES lv_vbak_count.
  DESCRIBE TABLE lt_vbap LINES lv_vbap_count.
  DESCRIBE TABLE lt_vbfa LINES lv_vbfa_count.

  WRITE: / 'ECC-like SD demo data loaded.'.
  WRITE: / 'Customers:', lv_kna1_count.
  WRITE: / 'Orders   :', lv_vbak_count.
  WRITE: / 'Items    :', lv_vbap_count.
  WRITE: / 'Doc flow :', lv_vbfa_count.
  WRITE: / 'Run ZSD_COCKPIT_ECC_LIKE to display the ALV cockpit.'.
ENDFORM.
