...
- Programme seed ECC-like: `ZSD_COCKPIT_ECC_SEED`
- Programme cockpit ECC-like sur tables Z: `ZSD_COCKPIT_ECC_LIKE`
...
- `ZSD_COCKPIT_ECC_SEED`: charge un jeu de donnees de test dans `ZSDC_KNA1`, `ZSDC_VBAK`, `ZSDC_VBAP`, `ZSDC_VBFA`.
- `ZSD_COCKPIT_ECC_LIKE`: affiche ces tables Z dans un ALV classique, avec double-clic commande -> postes et double-clic livraison/facture -> flux documentaire.
...
- `abap/src/zsd_cockpit_ecc_seed.prog.abap`
- `abap/src/zsd_cockpit_ecc_like.prog.abap`
- `abap/src/zsdc_kna1.tabl.xml`
- `abap/src/zsdc_vbak.tabl.xml`
- `abap/src/zsdc_vbap.tabl.xml`
- `abap/src/zsdc_vbfa.tabl.xml`
...
Pour le scenario autonome ECC-like sur tables Z:

1. Tables `ZSDC_*`
2. `ZSD_COCKPIT_ECC_SEED`
3. `ZSD_COCKPIT_ECC_LIKE`
...
## Version ECC-like autonome

`ZSD_COCKPIT_ECC_LIKE` est la variante conseillee pour un systeme de test sans donnees SD standard.

Elle utilise:

- `ZSDC_KNA1`: clients de demo.
- `ZSDC_VBAK`: entetes de commandes.
- `ZSDC_VBAP`: postes de commandes.
- `ZSDC_VBFA`: flux documentaire simplifie.

Execution:

1. Lancer `ZSD_COCKPIT_ECC_SEED` dans `SE38` ou `SA38`.
2. Verifier les donnees avec `SE16N` sur `ZSDC_*`.
3. Lancer `ZSD_COCKPIT_ECC_LIKE`.
4. Double-cliquer une commande pour afficher ses postes.
5. Double-cliquer une livraison ou facture pour afficher le flux documentaire.

Transaction optionnelle:

- `ZSD_COCKPIT_LIKE`
- Type: transaction de programme avec ecran de selection.
- Programme: `ZSD_COCKPIT_ECC_LIKE`.

Cette variante ne depend pas de RAP, CDS, OData, Fiori, Gateway ou BTP. Elle reste volontairement dans les outils classiques SAP GUI: `SE38`, `SA38`, `SE16N`, `SE93` et ALV.
